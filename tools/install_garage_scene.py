...
def ui_button(entity_id: str, name: str, parent: str, position, scale, fill, label: str, anchor="center") -> dict:
    return {
        "id": entity_id, "name": name, "parent": parent,
        "transform_3d": transform(position, scale),
        "components": {
            "sindri.ui.shape": {"kind": "rect", "fill": fill, "anchor": anchor, "layer": 102},
...
def add_car_layers(scene: dict, manifest: dict) -> None:
    scene["entities"] = [entity for entity in scene["entities"] if not str(entity.get("id", "")).startswith("car-layer-")]
    car = next(entity for entity in scene["entities"] if entity.get("id") == "parity-car")
    # Every generated car texture is now registered to the original 500x500
    # Flash stage using character 875's exact frame-730 matrix. The root must
    # therefore stay at the stage origin and each child uses the same 10x10
    # world canvas as the registered showroom layers.
    car["disabled"] = False
    car["transform_3d"] = transform((0.0, 0.0, 0.0), (1.0, 1.0, 1.0))
    car["components"] = {"sindri.tags": {"tags": ["car", "mujaffa-car", "showroom-registered"]}}
    ...
    def piece(key: str, label: str, layer: int, active: bool, tags: list[str]) -> None:
        ...
        generated.append({
            ...
            "parent": "parity-car",
            "disabled": not active,
            "transform_3d": transform((0.0, 0.0, 0.0), (10.0, 10.0, 1.0)),
            "components": {
                "sindri.sprite": {
    ...
    for slot in manifest["body_stack"]:
        ...
        for variant in variants:
            label = variant["label"]
            piece(variant["key"], f"{category}-{label}", layer, label == defaults[category], [category, safe_id(label)])
        layer += 1
    ...
    for tyre in manifest["tyres"]["variants"]:
        for rim in manifest["rims"]["variants"]:
            piece(
                f"hjulkapsel:{tyre}:{rim}", f"hjulkapsel-{tyre}-{rim}", 81,
...
def add_garage_ui(scene: dict) -> None:
    scene["entities"] = [
        ...
        if not str(entity.get("id", "")).startswith("garage-ui-") and entity.get("id") != "garage-controller"
    ]
    ...
    desktop_root = {"id": "garage-ui-desktop", "name": "Garage Desktop UI", "transform_3d": transform(), "components": {"sindri.ui.shape": {"kind": "rect", "fill": [0.0, 0.0, 0.0, 0.0], "anchor": "center", "layer": 100}}}
    mobile_root = {"id": "garage-ui-mobile", "name": "Garage Mobile UI", "disabled": True, "transform_3d": transform(), "components": {"sindri.ui.shape": {"kind": "rect", "fill": [0.0, 0.0, 0.0, 0.0], "anchor": "center", "layer": 100}}}
    ...
    for index, ((category, label), color) in enumerate(zip(CATEGORIES, palette)):
        y = 0.73 - index * 0.16
        scene["entities"].append(ui_button(f"garage-ui-desktop-category-{category}", f"Desktop Category {label}", "garage-ui-desktop", (-0.72, y, 0.0), (0.22, 0.115, 1.0), color, label))
        x = -0.405 + index * 0.09
        scene["entities"].append(ui_button(f"garage-ui-mobile-category-{category}", f"Mobile Category {label}", "garage-ui-mobile", (x, -0.84, 0.0), (0.075, 0.12, 1.0), color, label))
...
def main() -> int:
    ...
    stage_registration = project / "assets/generated/car-stage/stage-registration.json"
    if not manifest_path.is_file() or not stage_registration.is_file():
        raise SystemExit("generate runtime and stage-registered car art before installing the garage scene")
    scene = json.loads(scene_path.read_text(encoding="utf-8"))
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    add_car_layers(scene, manifest)
    add_garage_ui(scene)
    scene_path.write_text(json.dumps(scene, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print("installed responsive garage with car registered to original frame-730 placement")
    return 0
