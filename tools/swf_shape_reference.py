...
def _fill(bits: BitReader, shape_version: int) -> dict[str, object]:
    kind = bits.read_unsigned(8)
    alpha = shape_version >= 3
    if kind == 0x00:
        return {"type": "solid", "color": _rgba(bits, alpha)}
    if kind in {0x10, 0x12, 0x13}:
        matrix = _matrix(bits)
        spread = interpolation = 0
        if shape_version >= 4:
            spread, interpolation = bits.read_unsigned(2), bits.read_unsigned(2)
        else:
            bits.read_unsigned(4)
        count = bits.read_unsigned(4)
        stops = [{"ratio": bits.read_unsigned(8), "color": _rgba(bits, alpha)} for _ in range(count)]
        ...
        if kind == 0x13:
            result["focal_point"] = bits.read_signed(16) / 256.0
        return result
    if kind in {0x40, 0x41, 0x42, 0x43}:
        return {"type": "bitmap", "bitmap_id": bits.read_unsigned(16), "matrix": _matrix(bits), "repeat": kind in {0x40, 0x42}, "smooth": kind in {0x40, 0x41}}
...
def _line_array(bits: BitReader, shape_version: int) -> list[dict[str, object]]:
    ...
    for _ in range(count):
        ...
        if shape_version < 4:
            lines.append({"width": width, "color": _rgba(bits, shape_version >= 3)})
            continue
        ...
        if join == 2:
            line["miter_limit"] = bits.read_unsigned(16) / 256.0
        if has_fill:
            line["fill"] = _fill(bits, shape_version)
        else:
            line["color"] = _rgba(bits, True)
        lines.append(line)
    return lines
...
def decode_shape_records(payload: bytes, offset: int, shape_version: int) -> dict[str, object]:
    ...
    fills = _fill_array(bits, shape_version)
    lines = _line_array(bits, shape_version)
    fill_bits, line_bits = bits.read_unsigned(4), bits.read_unsigned(4)
    records = []
    x = y = 0
    while True:
        edge = bits.read_unsigned(1)
        if edge:
            ...
            if straight:
                ...
                cdx, cdy = bits.read_signed(n), bits.read_signed(n); adx, ady = bits.read_signed(n), bits.read_signed(n)
                cx, cy = x + cdx, y + cdy; x, y = cx + adx, cy + ady
        ...
        change = {"type": "style"}
        if flags & 1:
            n = bits.read_unsigned(5); x, y = bits.read_signed(n), bits.read_signed(n); change["move_to"] = [x / 20.0, y / 20.0]
        ...
        if flags & 16:
            change["new_fills"] = _fill_array(bits, shape_version); change["new_lines"] = _line_array(bits, shape_version); fill_bits, line_bits = bits.read_unsigned(4), bits.read_unsigned(4)
