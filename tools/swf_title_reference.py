...
def _cxform_alpha(p,o):
 b=BitReader(p,o); has_add=bool(b.read_unsigned(1)); has_mult=bool(b.read_unsigned(1)); n=b.read_unsigned(4)
 mult=[256]*4; add=[0]*4
 if has_mult: mult=[b.read_signed(n) for _ in range(4)]
 if has_add: add=[b.read_signed(n) for _ in range(4)]
 return {"multiply":{"r":mult[0]/256.,"g":mult[1]/256.,"b":mult[2]/256.,"a":mult[3]/256.},"add":{"r":add[0],"g":add[1],"b":add[2],"a":add[3]}},b.byte_offset
def _place(code,p):
 if code==4:
 ...
 if f&2:
  if o+2>len(p): raise SwfError("truncated character id")
  e["character_id"]=_u16(p,o); o+=2
 if f&4: e["matrix"],o=_matrix(p,o)
 if f&8: e["color_transform"],o=_cxform_alpha(p,o)
 if f&16:
  if o+2>len(p): raise SwfError("truncated ratio")
  e["ratio"]=_u16(p,o); o+=2
 if f&32:
  end=p.find(b"\0",o)
  if end<0: raise SwfError("unterminated name")
  e["name"]=p[o:end].decode("utf-8",errors="replace"); o=end+1
 if f&64:
  if o+2>len(p): raise SwfError("truncated clip depth")
  e["clip_depth"]=_u16(p,o); o+=2
...
def _trace(data,off,end):
 ...
 return out
def _state_through(trace,frame):
 state={}
 for e in trace:
  if int(e["frame"])>frame:break
  tag=str(e["tag"])
  if tag.startswith("PlaceObject"):
   d=int(e["depth"]); prior=state.get(d,{}) if e.get("move") else {}; state[d]={**prior,**{k:v for k,v in e.items() if k not in {"frame","tag","move"}}}
  elif tag.startswith("RemoveObject"):state.pop(int(e["depth"]),None)
...
def extract(path):
 ...
 labels.sort(key=lambda e:int(e["frame"]));trace=_trace(data,h.tags_offset,max(int(e["frame"]) for e in labels));snaps=[{"frame":int(e["frame"]),"label":e["label"],"display_list":_state_through(trace,int(e["frame"]))} for e in labels]
 return {"schema_version":7,"source":path.name,"stage":[h.width,h.height],"frame_rate":h.fps,"opening_labels":labels,"named_characters":_named(data,h.tags_offset),"title_relevant_tag_counts":_counts(data,h.tags_offset),"opening_display_trace":trace,"opening_display_snapshots":snaps,"notes":["Placement matrices use pixels; SWF translation twips are divided by 20.","PlaceObject2 color transforms are decoded to normalized multiply terms and byte-space additive terms.","Display snapshots resolve depth replacement and move updates at each opening label.","This file deliberately contains no workshop economy data."]}
def main():
